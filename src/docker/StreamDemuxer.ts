...
export class StreamDemuxer {
  ...
  private isValidHeader(header: Buffer): boolean {
    ...
    return (streamType === 1 || streamType === 2) && 
           zeros.every(b => b === 0) &&
           size > 0 && size <= StreamDemuxer.MAX_FRAME_SIZE;
  }
  ...
  private async detectMultiplexed(buffer: Buffer): Promise<boolean> {
    ...
    if (buffer.length >= secondFrameStart + StreamDemuxer.HEADER_SIZE) {
      const secondHeader = buffer.slice(secondFrameStart, secondFrameStart + StreamDemuxer.HEADER_SIZE);
      ...
      for await (const chunk of stream) {
        ...
        if (isMultiplexed === false) {
          ...
          while (processed && buffer.length >= StreamDemuxer.HEADER_SIZE) {
            ...
            // Extract frame
            const payload = buffer.slice(StreamDemuxer.HEADER_SIZE, StreamDemuxer.HEADER_SIZE + payloadSize);
            ...
            while (offset < payload.length) {
              const chunkData = payload.slice(offset, Math.min(offset + chunkSize, payload.length)).toString('utf8');
